...
import { CheckCircle, XCircle, Clock, AlertCircle, Save, Loader2, Calendar, MapPin, Users } from 'lucide-react';
...
const MarkAvailabilityPage = () => {
  ...
  // Fetch meeting details and current availability
  useEffect(() => {
    let isMounted = true;
  ...
  const handleSubmit = async (e) => {
    ...
    // Validate role preferences if available
    if (selectedStatus === 'AVAILABLE') {
      if (rolePreferences.length < 1) {
        toast.error('Please select at least 1 role preference');
        return;
      }
      if (rolePreferences.length > 3) {
        toast.error('Please select at most 3 role preferences');
        return;
      }
    }
        ...
        <div className="p-6 border-b border-gray-200">
          ...
          <div className="mt-4 space-y-2 text-sm text-gray-600">
            ...
            <div className="flex items-center">
              ...
              <span>{meeting.startTime} - {meeting.endTime || 'End Time'}</span>
            </div>
...
export default MarkAvailabilityPage;
